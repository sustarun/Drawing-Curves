...
(define (rotate-around-origin radians curve)
  (lambda (t)
    (let ((ct (curve t))
          (cos-val (cos radians))
          (sin-val (sin radians)))
      (make-point (- (* cos-val (x-of ct)) (* sin-val (y-of ct)))
                  (+ (* cos-val (y-of ct)) (* sin-val (x-of ct)))))))
...
(define (connect-ends curve1 curve2)
  (let* [(trans-factor-x (- ((curve1 1) 0) ((curve2 0) 0)))
         (trans-factor-y (- ((curve1 1) 1) ((curve2 0) 1)))]
    (lambda (t)
      (if (< t 0.5) (curve1 (* 2 t))
          ((translate trans-factor-x trans-factor-y curve2) (- (* 2 t) 1))))))
...
(define (gosper-general curve calc-angle l)
  (gosper-help curve calc-angle l))

(define (gosper-help curve calc-angle l)
  (if (= l 0) curve
      (let* [(angle (calc-angle l))
              (scale-factor (/ 0.5 (cos angle)))
              (curve-scale (scale scale-factor scale-factor (gosper-help curve calc-angle (- l 1))))
              (curve1 (rotate-around-origin angle curve-scale))
              (curve2 (rotate-around-origin (- angle) curve-scale))]
         (connect-ends curve1 curve2))))
...
(define (sierpinski-curve level)
  (let* [(angle (/ pi 3))
         (curve (connect-rigidly (connect-ends (rotate-around-origin angle (unit-line))
                                           (rotate-around-origin (- angle) (unit-line)))
                                (unit-line)))]
  ((repeated sierpinski-step level) curve)))

(define (sierpinski-step curve)
  (let* [(scale-curve (scale 0.5 0.5 curve))]
    (connect-rigidly scale-curve
                     (connect-rigidly (translate 0.25 (/ (sqrt 3) 4) scale-curve)
                                      (translate 0.5 0 scale-curve)))))