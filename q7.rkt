...
(define (koch-curve level)
  (let* [(element (unit-line))
         (angle (/ pi 3))
        (curve (connect-rigidly (connect-ends (rotate-around-origin angle element)
                                           (rotate-around-origin (- angle) element))
                                element))]
  (car ((repeated koch-step level) (cons curve element)))))

(define (koch-step curv-el)
  (let* [(scale-factor (/ 1 3))
         (angle (/ pi 3))
         (element (scale scale-factor scale-factor (connect-ends
                                                    (connect-ends
                                                     (connect-ends (cdr curv-el)
                                                                   (rotate-around-origin angle (cdr curv-el)))
                                                     (rotate-around-origin (- angle) (cdr curv-el)))
                                                    (cdr curv-el))))
         (curve (connect-ends (connect-ends (rotate-around-origin angle element)
                                            (rotate-around-origin (- angle) element))
                              (rotate-around-origin pi element)))]
    (cons curve element)))